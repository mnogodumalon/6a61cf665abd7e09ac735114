...
interface EventVerwaltungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: EventVerwaltung['fields']) => Promise<void>;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  event_category: LOOKUP_OPTIONS['event_verwaltung']?.['event_category'] ?? [],
  skill_level: LOOKUP_OPTIONS['event_verwaltung']?.['skill_level'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  location: APP_IDS.SKATEPARKS_VERANSTALTUNGSORTE,
};
...
export function EventVerwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, skateparksVeranstaltungsorteList, enablePhotoScan = true, enablePhotoLocation = true }: EventVerwaltungDialogProps) {
  const [fields, setFields] = useState<Partial<EventVerwaltung['fields']>>({});
  ...
  // Inline-Create state for "SkateparksVeranstaltungsorte" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraSkateparksVeranstaltungsorte` list, and select it in
  // the originating Combobox via the captured `createSkateparksVeranstaltungsorteField`.
  const [createSkateparksVeranstaltungsorteOpen, setCreateSkateparksVeranstaltungsorteOpen] = useState(false);
  const [createSkateparksVeranstaltungsorteInitial, setCreateSkateparksVeranstaltungsorteInitial] = useState('');
  const [createSkateparksVeranstaltungsorteField, setCreateSkateparksVeranstaltungsorteField] = useState<string>('');
  const [extraSkateparksVeranstaltungsorte, setExtraSkateparksVeranstaltungsorte] = useState< SkateparksVeranstaltungsorte[]>([]);
  const skateparksVeranstaltungsorteListAll = useMemo(
    () => [...skateparksVeranstaltungsorteList, ...extraSkateparksVeranstaltungsorte],
    [skateparksVeranstaltungsorteList, extraSkateparksVeranstaltungsorte],
  );
  function openCreateSkateparksVeranstaltungsorte(fieldKey: string, q: string) {
    setCreateSkateparksVeranstaltungsorteField(fieldKey);
    setCreateSkateparksVeranstaltungsorteInitial(q);
    setCreateSkateparksVeranstaltungsorteOpen(true);
  }
  const [showErrors, setShowErrors] = useState(false);
  const REQUIRED_FIELDS = ['event_title', 'event_category', 'event_datetime', 'skill_level', 'location', 'organizer_firstname', 'organizer_lastname', 'organizer_email'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<EventVerwaltung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'event_verwaltung');
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="location" entity="Skateparks & Veranstaltungsorte">\n${JSON.stringify(skateparksVeranstaltungsorteList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["location"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const locationName = raw['location'] as string | null;
        if (locationName) {
          const locationMatch = skateparksVeranstaltungsorteList.find(r => matchName(locationName!, [String(r.fields.location_name ?? '')]));
          if (locationMatch) merged['location'] = createRecordUrl(APP_IDS.SKATEPARKS_VERANSTALTUNGSORTE, locationMatch.record_id);
        }
        return merged as Partial<EventVerwaltung['fields']>;
      });
      ...
      if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
        try {
          ...
          setFields(prev => ({ ...prev, event_flyer: fileUrl }));
        } catch (uploadErr) {
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'organizer_notes': (
      <div key="organizer_notes" className="space-y-1.5">
        <Label htmlFor="organizer_notes">{fieldLabel('event_verwaltung', 'organizer_notes')}</Label>
        <Textarea
          id="organizer_notes"
          placeholder="Organisatorische Notizen, To-Dos, Bemerkungen..."
    ...
    'event_title': (
      <div key="event_title" className="space-y-1.5">
        <Label htmlFor="event_title">{fieldLabel('event_verwaltung', 'event_title')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="event_title"
          placeholder="z. B. Berlin Skate Cup 2026"
          value={fields.event_title ?? ''}
          onChange={e => setFields(f => ({ ...f, event_title: e.target.value }))}
          required
        />
        {showErrors && !fields.event_title && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'event_description': (
      <div key="event_description" className="space-y-1.5">
        <Label htmlFor="event_description">{fieldLabel('event_verwaltung', 'event_description')}</Label>
        <Textarea
          id="event_description"
          placeholder="Ablauf, Format, Attraktionen, Besonderheiten..."
    ...
    'event_category': (
      <div key="event_category" className="space-y-1.5">
        <Label htmlFor="event_category">{fieldLabel('event_verwaltung', 'event_category')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('event_verwaltung', 'event_category', 'contest') ?? 'Contest'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('event_verwaltung', 'event_category', 'jam_session') ?? 'Jam Session'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('event_verwaltung', 'event_category', 'demo') ?? 'Demo'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('event_verwaltung', 'event_category', 'workshop') ?? 'Workshop'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('event_verwaltung', 'event_category', 'sonstiges') ?? 'Sonstiges'}
          </button>
        </div>
    ...
    'event_datetime': (
      <div key="event_datetime" className="space-y-1.5">
        <Label htmlFor="event_datetime">{fieldLabel('event_verwaltung', 'event_datetime')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="event_datetime"
          ...
          required
        />
        {showErrors && !fields.event_datetime && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'skill_level': (
      <div key="skill_level" className="space-y-1.5">
        <Label htmlFor="skill_level">{fieldLabel('event_verwaltung', 'skill_level')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('event_verwaltung', 'skill_level', 'beginner') ?? 'Anfänger'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('event_verwaltung', 'skill_level', 'intermediate') ?? 'Fortgeschrittene'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('event_verwaltung', 'skill_level', 'advanced') ?? 'Profis'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('event_verwaltung', 'skill_level', 'all_levels') ?? 'Alle Levels'}
          </button>
        </div>
    ...
    'max_participants': (
      <div key="max_participants" className="space-y-1.5">
        <Label htmlFor="max_participants">{fieldLabel('event_verwaltung', 'max_participants')}</Label>
        <Input
          id="max_participants"
          type="number"
          step="any"
    ...
    'entry_fee': (
      <div key="entry_fee" className="space-y-1.5">
        <Label htmlFor="entry_fee">{fieldLabel('event_verwaltung', 'entry_fee')}</Label>
        <Input
          id="entry_fee"
          type="number"
          step="any"
    ...
    'location': (
      <div key="location" className="space-y-1.5">
        <Label htmlFor="location">{fieldLabel('event_verwaltung', 'location')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="location"
          placeholder="Wo ist der Veranstaltungsort?"
          items={skateparksVeranstaltungsorteListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.location_name ?? r.record_id),
          }))}
          value={extractRecordId(fields.location)}
          onChange={id => setFields(f => ({ ...f, location: id ? createRecordUrl(APP_IDS.SKATEPARKS_VERANSTALTUNGSORTE, id) : undefined }))}
          onCreateNew={(q) => openCreateSkateparksVeranstaltungsorte("location", q)}
          createLabel={t('create_in', { entity: appLabel('skateparks_veranstaltungsorte') })}
        />
        {showErrors && !fields.location && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'organizer_firstname': (
      <div key="organizer_firstname" className="space-y-1.5">
        <Label htmlFor="organizer_firstname">{fieldLabel('event_verwaltung', 'organizer_firstname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          ...
          required
        />
        {showErrors && !fields.organizer_firstname && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'organizer_lastname': (
      <div key="organizer_lastname" className="space-y-1.5">
        <Label htmlFor="organizer_lastname">{fieldLabel('event_verwaltung', 'organizer_lastname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          ...
          required
        />
        {showErrors && !fields.organizer_lastname && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'organizer_email': (
      <div key="organizer_email" className="space-y-1.5">
        <Label htmlFor="organizer_email">{fieldLabel('event_verwaltung', 'organizer_email')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="organizer_email"
          type="email"
          placeholder="z. B. tom@beispiel.de"
          value={fields.organizer_email ?? ''}
          onChange={e => setFields(f => ({ ...f, organizer_email: e.target.value }))}
        />
        {showErrors && !fields.organizer_email && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'organizer_phone': (
      <div key="organizer_phone" className="space-y-1.5">
        <Label htmlFor="organizer_phone">{fieldLabel('event_verwaltung', 'organizer_phone')}</Label>
        <Input
          id="organizer_phone"
    ...
    'event_flyer': (
      <div key="event_flyer" className="space-y-1.5">
        <Label htmlFor="event_flyer">{fieldLabel('event_verwaltung', 'event_flyer')}</Label>
        {fields.event_flyer ? (
          <div className="flex items-center gap-3 rounded-lg border p-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm truncate text-foreground">{fields.event_flyer.split("/").pop()}</p>
              <div className="flex gap-2 mt-1">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setFields(f => ({ ...f, event_flyer: fileUrl }));
                ...
                <button
                  ...
                  onClick={() => setFields(f => ({ ...f, event_flyer: undefined }))}
                >
                  {t('fr_remove')}
                </button>
              </div>
  ...
  const FIELD_LABELS: Record<string, string> = {"organizer_notes": "Notizen", "event_title": "Titel des Events", "event_description": "Beschreibung", "event_category": "Kategorie", "event_datetime": "Datum und Uhrzeit", "skill_level": "Schwierigkeitsgrad", "max_participants": "Maximale Teilnehmerzahl", "entry_fee": "Startgebühr (in €)", "location": "Veranstaltungsort", "organizer_firstname": "Vorname des Organisators", "organizer_lastname": "Nachname des Organisators", "organizer_email": "E-Mail des Organisators", "organizer_phone": "Telefonnummer des Organisators", "event_flyer": "Event-Flyer"};
  const CURRENCY_KEYS = new Set<string>(["entry_fee"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"location": {"location_name": "Name des Veranstaltungsorts", "street": "Straße", "house_number": "Hausnummer", "postal_code": "Postleitzahl", "city": "Stadt", "description": "Beschreibung", "special_notes": "Besondere Hinweise", "location_photo": "Foto des Veranstaltungsorts"}};
  ...
  return (
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.EVENT_VERWALTUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createSkateparksVeranstaltungsorteOpen && (
      <SkateparksVeranstaltungsorteDialog
        open={createSkateparksVeranstaltungsorteOpen}
        onClose={() => setCreateSkateparksVeranstaltungsorteOpen(false)}
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createSkateparksVeranstaltungsorteEntry(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as SkateparksVeranstaltungsorte;
            setExtraSkateparksVeranstaltungsorte(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.SKATEPARKS_VERANSTALTUNGSORTE, result.id);
            setFields(prev => ({ ...prev, [createSkateparksVeranstaltungsorteField]: url } as any));
          }
          setCreateSkateparksVeranstaltungsorteOpen(false);
        }}
        defaultValues={createSkateparksVeranstaltungsorteInitial
          ? ({ location_name: createSkateparksVeranstaltungsorteInitial } as any)
          : undefined}
      />
    )}
    </>
