...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/SkateparksVeranstaltungsorte';
...
interface SkateparksVeranstaltungsorteDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: SkateparksVeranstaltungsorte['fields']) => Promise<void>;
...
export function SkateparksVeranstaltungsorteDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: SkateparksVeranstaltungsorteDialogProps) {
  const [fields, setFields] = useState<Partial<SkateparksVeranstaltungsorte['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['location_name', 'street', 'house_number', 'postal_code', 'city'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<SkateparksVeranstaltungsorte['fields']>);
      setPreview(null);
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "location_name": string | null, // Name des Veranstaltungsorts\n  "street": string | null, // Straße\n  "house_number": string | null, // Hausnummer\n  "postal_code": string | null, // Postleitzahl\n  "city": string | null, // Stadt\n  "description": string | null, // Beschreibung\n  "special_notes": string | null, // Besondere Hinweise\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<SkateparksVeranstaltungsorte['fields']>;
      });
      ...
      if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
        try {
          ...
          setFields(prev => ({ ...prev, location_photo: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('skateparks_veranstaltungsorte') })
    : t('new_entity', { entity: appLabel('skateparks_veranstaltungsorte') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'location_name': (
      <div key="location_name" className="space-y-1.5">
        <Label htmlFor="location_name">{fieldLabel('skateparks_veranstaltungsorte', 'location_name')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="location_name"
          placeholder="z. B. Skate-Park Döhren"
          value={fields.location_name ?? ''}
          onChange={e => setFields(f => ({ ...f, location_name: e.target.value }))}
          required
        />
        {showErrors && !fields.location_name && (
    ...
    'street': (
      <div key="street" className="space-y-1.5">
        <Label htmlFor="street">{fieldLabel('skateparks_veranstaltungsorte', 'street')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="street"
          placeholder="z. B. Langenberger Straße"
          value={fields.street ?? ''}
          onChange={e => setFields(f => ({ ...f, street: e.target.value }))}
          required
        />
        {showErrors && !fields.street && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'house_number': (
      <div key="house_number" className="space-y-1.5">
        <Label htmlFor="house_number">{fieldLabel('skateparks_veranstaltungsorte', 'house_number')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="house_number"
          placeholder="z. B. 42a"
          ...
          required
        />
        {showErrors && !fields.house_number && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'postal_code': (
      <div key="postal_code" className="space-y-1.5">
        <Label htmlFor="postal_code">{fieldLabel('skateparks_veranstaltungsorte', 'postal_code')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="postal_code"
          placeholder="z. B. 30419"
          value={fields.postal_code ?? ''}
          onChange={e => setFields(f => ({ ...f, postal_code: e.target.value }))}
          required
        />
        {showErrors && !fields.postal_code && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'city': (
      <div key="city" className="space-y-1.5">
        <Label htmlFor="city">{fieldLabel('skateparks_veranstaltungsorte', 'city')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="city"
    ...
    'description': (
      <div key="description" className="space-y-1.5">
        <Label htmlFor="description">{fieldLabel('skateparks_veranstaltungsorte', 'description')}</Label>
        <Textarea
          id="description"
          placeholder="Ausstattung, Flächen, besondere Features..."
    ...
    'special_notes': (
      <div key="special_notes" className="space-y-1.5">
        <Label htmlFor="special_notes">{fieldLabel('skateparks_veranstaltungsorte', 'special_notes')}</Label>
        <Textarea
          id="special_notes"
          placeholder="Öffnungszeiten, Parkplätze, Anforderungen..."
    ...
    'location_photo': (
      <div key="location_photo" className="space-y-1.5">
        <Label htmlFor="location_photo">{fieldLabel('skateparks_veranstaltungsorte', 'location_photo')}</Label>
        {fields.location_photo ? (
          <div className="flex items-center gap-3 rounded-lg border p-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm truncate text-foreground">{fields.location_photo.split("/").pop()}</p>
              <div className="flex gap-2 mt-1">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setFields(f => ({ ...f, location_photo: fileUrl }));
                ...
                <button
                  ...
                  onClick={() => setFields(f => ({ ...f, location_photo: undefined }))}
                >
                  {t('fr_remove')}
                </button>
              </div>
  ...
  const FIELD_LABELS: Record<string, string> = {"location_name": "Name des Veranstaltungsorts", "street": "Straße", "house_number": "Hausnummer", "postal_code": "Postleitzahl", "city": "Stadt", "description": "Beschreibung", "special_notes": "Besondere Hinweise", "location_photo": "Foto des Veranstaltungsorts"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.SKATEPARKS_VERANSTALTUNGSORTE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
