  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t('view_entity', { entity: appLabel('skateparks_veranstaltungsorte') })}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('skateparks_veranstaltungsorte', 'location_name')}</Label>
            <p className="text-sm">{record.fields.location_name ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('skateparks_veranstaltungsorte', 'street')}</Label>
            <p className="text-sm">{record.fields.street ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('skateparks_veranstaltungsorte', 'house_number')}</Label>
            <p className="text-sm">{record.fields.house_number ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('skateparks_veranstaltungsorte', 'postal_code')}</Label>
            <p className="text-sm">{record.fields.postal_code ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('skateparks_veranstaltungsorte', 'city')}</Label>
            <p className="text-sm">{record.fields.city ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('skateparks_veranstaltungsorte', 'description')}</Label>
            <p className="text-sm whitespace-pre-wrap">{record.fields.description ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('skateparks_veranstaltungsorte', 'special_notes')}</Label>
            <p className="text-sm whitespace-pre-wrap">{record.fields.special_notes ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('skateparks_veranstaltungsorte', 'location_photo')}</Label>
